...
class AkkaHttpServer(
  ...
  private def createServerBinding(port: Int, connectionContext: ConnectionContext): Http.ServerBinding = {
    ...
    val serverSource: Source[Http.IncomingConnection, Future[Http.ServerBinding]] =
      Http().bind(interface = config.address, port, connectionContext)

    val connectionSink: Sink[Http.IncomingConnection, _] = Sink.foreach { connection: Http.IncomingConnection =>
      connection.handleWithAsyncHandler(handleRequest(connection.remoteAddress, _, connectionContext))
    }
  ...
  private val httpsServerBinding = config.sslPort.map { port =>
    val connectionContext = try {
      ...
      case NonFatal(e) =>
        ...
        // fall back to http
        HttpConnectionContext.getInstance
    }
  ...
  private def handleRequest(remoteAddress: InetSocketAddress, request: HttpRequest, connectionContext: ConnectionContext): Future[HttpResponse] = {
    val requestId = requestIDs.incrementAndGet()
    val (convertedRequestHeader, requestBodySource) = modelConversion.convertRequest(
      requestId = requestId,
      remoteAddress = remoteAddress,
      secureProtocol = connectionContext.isSecure,
      request = request)
